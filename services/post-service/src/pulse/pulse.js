...
// Unicode-aware — supports Bangla, English, mixed tags
// .slice(1) removes the leading # so trending shows "মনখারাপ" not "#মনখারাপ"
...
export const pulse = {
  ...
  getSignal() {
    ...
    const signal =
      total >= 5 && rate >= 2.0 ? { type: 'surge',  message: '⚡ Engagement surge detected' }
    : total >= 6 && rate >= 1.0 ? { type: 'rising', message: '📈 Community activity rising' }
    : total >= 3 && rate >= 0.3 ? { type: 'active', message: '📈 Community is active' }
    :                              { type: 'normal', message: '📈 Community is calm' };
